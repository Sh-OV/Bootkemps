...
int[,] MatrixGenerator(int rows, int columns)       // Создание матрицы случайных чисел
{
...
void SerialMatrixMul(int[,] a, int[,] b)        // метод умножения матриц в однопотоке
{
...
void PrepareParallelMatrixMul(int[,] a, int[,] b)        // метод подготовления к параллельному умножению матриц
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы");
...
void ParallelMatrixMul(int[,] a, int[,] b, int startPos, int endPos)        // метод паралельного умножения матриц
{
